...
validators.anyOf = function validateAnyOf (instance, schema, options, ctx) {
  ...
  var allResults = [];
  if (!schema.anyOf.some(function(schema) {
    var res = this.validateSchema(instance, schema, options, ctx);
    allResults.push(res);
    return res.valid;
  }, this)) {
    allResults.forEach(function(r) { 
      if (!r.valid) result.importErrors(r); 
    });
...
validators.oneOf = function validateOneOf (instance, schema, options, ctx) {
  ...
  var result = new ValidatorResult(instance, schema, options, ctx);
  var self = this;
  var allResults = schema.oneOf.map(function(s) { return this.validateSchema(instance, s, options, ctx); }, this);
  var count = allResults.filter(function(r) { return r.valid; }).length;
  if (count!==1) {
    ...
    result.addError({
      name: 'oneOf',
      argument: list,
      message: "matches " + count + ", not exactly one from " + list.join(','),
    });
    if (count === 0) allResults.forEach(function(r) { 
      if (!r.valid) result.importErrors(r); 
    });
  }
  return result;
...
validators.not = validators.disallow = function validateNot (instance, schema, options, ctx) {
  ...
  notTypes.forEach(function (type) {
    if (self.testType(instance, schema, options, ctx, type)) {
      var schemaId = type && type.id && ('<' + type.id + '>') || JSON.stringify(type);
      result.addError({
        name: 'not',
        argument: schemaId,
